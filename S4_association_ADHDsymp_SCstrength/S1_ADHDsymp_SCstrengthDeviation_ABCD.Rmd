...
for (VOI in symptomvar){
  if (!file.exists(paste0(interfileFolder, '/SCDeviation_', VOI,'_Age_CV75_Yeo', Yeoresolution,'_ADHDall_controlAgeSex.rds'))){
    ...
    resultsum <- mclapply(1:element_num, function(x){
      ...
      #gamdata <- SCdata.ADHD[SCdata.ADHD[[region]]<0, ]
      #assign("gamdata", gamdata, envir = globalenv())
...
for (VOI in symptomvar){
  ...
  # within edges with significant diagnostic effects during a period. (17)
  resultsum.df3 <- resultsum.df
  resultsum.df3$T.disease[!(resultsum.df3$int_var %in% sigedge.all)] <- NA
  resultsum.df3$bootstrap.P.disease[!(resultsum.df3$int_var %in% sigedge.all)] <- NA
  resultsum.df3$bootstrap.P.fdr <- p.adjust(resultsum.df3$bootstrap.P.disease, method="fdr")
  resultsum.df3$bootstrap.P.fdr[!(resultsum.df3$int_var %in% sigedge.all)] <- 1
...
symass.all <- rbind(symass.EXT, symass.ADHD, symass.ATT) %>% filter(int_var %in% sigedge.all)
...
# -0.054619  0.001694  0.026702  0.031366  0.063439  0.136204  

# -0.008253  0.025430  0.055658  0.055943  0.085110  0.136204 
...
j=0
for (VOI in symptomvar){
  resultsum.df <- readRDS(paste0(interfileFolder, '/SCDeviation_', VOI,'_Age_CV75_Yeo', Yeoresolution,'_TD_controlAgeSex.rds'))
  SC_symptom.list[[VOI]] <- resultsum.df
  ## All 120 edges
  # symptom effects
  resultsum.df$bootstrap.P.fdr <- p.adjust(resultsum.df$bootstrap.P.disease, method="fdr")
...
SC_symptom.adhd <- SC_symptom.list[["cbcl_scr_dsm5_adhd_t"]]
SC_symptom.external <- SC_symptom.list[["cbcl_scr_syn_external_t"]]
