...
result_path <- file.path(homepath, "data", "demography")
demo_path <- file.path(homepath, "data", "behavior")
qcpath <- file.path(homepath, "data", "qc")

basic_demo <- read.xlsx(file.path(demo_path, "basic_demography20250418.xlsx"), detectDates=T)
basic_demo$age <- as.numeric(difftime(basic_demo$Visitday, basic_demo$Birthday, units = "days") / 365)
basic_demo2 <- read.xlsx(file.path(demo_path, "basic_demography20241102.xlsx"), detectDates=T)
