...
demo_path <- "D:/xuxiaoyu/open_dataset_information/CCNP/devCCNP"
qcpath <- "D:/xuxiaoyu/open_dataset_information/CCNP/devCCNP/qc_json"
...
Include samples with age in 6~17 years old.
```{r age_sex}
basic_demo <- basic_demo %>% drop_na(Sex, ScanAge)
basic_demo <- basic_demo %>% filter(ScanAge > 6, ScanAge < 17)
allnum4 <- nrow(basic_demo)

print(paste((allnum3-allnum4), "participants were excluded due to age out of range.", allnum4, "scans left."))
```
...
basic_demo <- basic_demo %>% drop_na(mean_fd) %>% filter(mean_fd < mean(mean_fd)+3*sd(mean_fd))
