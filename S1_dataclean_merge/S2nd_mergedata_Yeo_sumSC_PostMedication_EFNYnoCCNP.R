...
Behavior <- Behavior %>% filter(age >= 6.5, age <= 15.5) # N=784
...
for (i in 1:nrow(Behavior)){
  ...
  if (file.exists(SC_file_path)){
    ...
    SCdat25_noInvNode$ID[1] <- ID
    print(i)
    SCdata.sum[[i]] <- data.frame(SCdat75, SCdat25, SCdat75_noInvNode, SCdat25_noInvNode)
  }
...
SCdata.sum25 <- do.call(rbind, lapply(SCdata.sum2, function(x) as.data.frame(x[,c((elementnum+2):(elementnum*2+2))])))
names(SCdata.sum25) <- c(colname, "ID")

SCdata.sum75_noInvNode <- do.call(rbind, lapply(SCdata.sum2, function(x) as.data.frame(x[,c((elementnum*2+3):(elementnum*3+3))])))
SCdata.sum25_noInvNode <- do.call(rbind, lapply(SCdata.sum2, function(x) as.data.frame(x[,c((elementnum*3+4):(elementnum*4+4))])))
names(SCdata.sum25_noInvNode) <- c(colname, "ID")
names(SCdata.sum75_noInvNode) <- c(colname, "ID")

SCdata.sum75.merge <- merge(SCdata.sum75, Behavior, by="ID") %>% drop_na(SC.1)
SCdata.sum25.merge <- merge(SCdata.sum25, Behavior, by="ID")
SCdata.sum75_noInvNode.merge <- merge(SCdata.sum75_noInvNode, Behavior, by="ID")
SCdata.sum25_noInvNode.merge <- merge(SCdata.sum25_noInvNode, Behavior, by="ID")
...
SCdata.sum25.merge$ID <- as.factor(SCdata.sum25.merge$ID) ; SCdata.sum25.merge$site <- as.factor(SCdata.sum25.merge$site)
...
SCdata.sum25_noInvNode.merge$ID <- as.factor(SCdata.sum25_noInvNode.merge$ID) ; SCdata.sum25_noInvNode.merge$site <- as.factor(SCdata.sum25_noInvNode.merge$site)
...
saveRDS(SCdata.sum25.merge, paste0(interfileFolder, '/SCdata_Yeo',Yeoresolution, '_CV25_sumSCinvnode.sum.msmtcsd.merge_postmedication.rds'))
...
saveRDS(SCdata.sum25_noInvNode.merge, paste0(interfileFolder, '/SCdata_Yeo',Yeoresolution, '_CV25_sumSC.sum.msmtcsd.merge_postmedication.rds'))
